...
def load_comp(input_path, output_dir):
    ...
    outfile = os.path.join(output_dir, "comp_bnd.csv")
...
def fill_in_gap(bedfile, fai_path, outdir, int_chrom_set, leafnodes):
    ...
    outfile = os.path.join(outdir, "bench_no_gap.bed")
    write_bnds(final_list ,outfile,'bed' )
    print(f"filled {gap_cnt} gaps in bench bed")
    return final_list


def load_bench(bench_dir, outdir, fai_path):
    bedfile = os.path.join(bench_dir, "gt_all.bed")
    leafnodefile = os.path.join(bench_dir, "all_leaf_node")
    ...
    bed_list = fill_in_gap(bedfile, fai_path, outdir, int_chrom_set, leafnodes)
    ...
    write_bnds(bnd_list, os.path.join(outdir, "bench_bnd.csv"), "bench")

    return bnd_list, os.path.join(outdir, "bench_bnd.csv")
...
def eval_cnv(bench_dir, comp_file, outdir, max_dist_thresh, fai_path, match_cn):

    comp_list , comp_outfile = load_comp(comp_file, outdir)

    bench_list , bench_outfile = load_bench(bench_dir, outdir, fai_path)
    # print(comp_list[0])
    # print(bench_list[0])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--fai_path', '-f', required=True, help="Path to genome FASTA index (.fai) used to fill benchmark gaps.")
    ...
    if not os.path.exists(fai_path):
        print(f"{fai_path} not exists.")
        exit()


    eval_cnv(bench_dir, comp_file, output_dir , max_dist_thresh, fai_path, match_cn = True)
    eval_cnv(bench_dir, comp_file, output_dir , max_dist_thresh, fai_path, match_cn = False)
