...
def collect_data(p, tool_list, eval_dir_list, data_type, max_dist_thresh):
    # tool_list = [
    #              "ginkgo",
    #              "hmmcopy",
    #              "scope",
    ...
    if data_type == 'CNV':
        try:
            file_list = [ eval_dir_list[i]+f'/stats_by_cell_{max_dist_thresh}_matchcn.csv' for i in range(len(tool_list))]
        except:
            print(len(eval_dir_list))
            print(len(tool_list))
    else:
        file_list = [ eval_dir_list[i]+f'/stats_by_cell_{max_dist_thresh}.csv' for i in range(len(tool_list))]
    precision_list =[]
    recall_list =[]
    f1_list = []
    name_list = []
...
def plot_violin(data_type, tool_list, p_list, df_config, out_dir, max_dist_thresh):
    '''
    data_type: 'CNV' 'SEG'
    '''
    ...
    for i in p_list:
        ...
        print(tool_list_p)
        dc_recall, dc_precision, dc_f1, df_new = collect_data(i,  tool_list_p, eval_dir_list, data_type, max_dist_thresh)
        if cnt == 0:
            df = df_new
        else:
...
tool_list = [ 'HMMcopy', 'rcCAE', 'Ginkgo', 'AneuFinder', 'SeCNV', 'SCOPE', 'CNVeil','Our method']
colors = [ '#ff7f0e', '#2ca02c', '#d62728', '#9467bd', '#8c564b', '#1f77b4' ,'#e377c2','#e377c2']

color_dict = dict(zip(tool_list, colors))

def main(input_path, output_dir, max_dist_thresh):
    ...
    print(tool_list)
    plot_violin('CNV', tool_list, p_list, df_config, output_dir, max_dist_thresh)
    plot_violin( 'SEG', tool_list, p_list, df_config, output_dir, max_dist_thresh)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input_path','-i', required=True, help = "a csv file, columns = tool,ploidy,eval_dir")
    parser.add_argument('--output_dir','-o', required=True)
    parser.add_argument('--max_dist_thresh', '-d', type=int, default=2000000)
    args = parser.parse_args()

    main(args.input_path, args.output_dir, args.max_dist_thresh)
