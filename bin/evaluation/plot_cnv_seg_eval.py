...
def plot_violin(data_type, tool_list, p_list, df_config, out_dir):
    ...
    g = sns.FacetGrid(df, row="metric", sharey=False,  aspect=3)
    # print(df)
    g = g.map(sns.violinplot, "Ploidy", "value", "Tool", cut=0, inner=None, scale="width", split=False, palette="muted", saturation=1).despine(left=True)
    ...
    fig_file = out_dir + f"/eval_simulation_combined_quan_{data_type}.png"
    fig_file1 = out_dir + f"/eval_simulation_combined_quan_{data_type}.pdf"
