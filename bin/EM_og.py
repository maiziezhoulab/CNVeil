...
def phase_one_chrom(df, count_file, chrom, cell_cluster,  snp_source, normal_cells, convert= None , snp_file = None, logger = None):
...
def run_one_chrom(i, baf_dir, sample, snp_source,df,convert,snp_file, cell_cluster_file, normal_cells,  logger):
    chrom = f"chr{i}"
    ...
    out = phase_one_chrom(df, count_file, chrom, cell_cluster_file, snp_source, normal_cells,  convert, snp_file, logger)
    return out
...
def allele_CN(total_cn_dir, outdir, baf_dir, sample, convert, snp_source, snp_file,n_thread = 10 ):
    ...
    if n_thread == 1:
        ...
        for i in tqdm(useful_is[-2:]):
            i=20
            logger.info(f"============={i}")
            out = run_one_chrom(i, baf_dir, sample, snp_source,df,convert,snp_file, cell_cluster_file,normal_cells, logger)
            outs.append(out)
            # exit()
            break
    else:
        outs = Parallel(n_jobs=n_thread)(delayed(run_one_chrom)(i,baf_dir, sample, snp_source, df, convert, snp_file, cell_cluster_file, normal_cells,  logger) for i in tqdm(useful_is))
